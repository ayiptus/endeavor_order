...
export async function POST(request: NextRequest) {
  try {
    ...
    const { clientInfo, cartItems, requestNumber, total, isTest } = body
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 20px;">
  ...
  <div style="border-top: 2px solid #e5e7eb; padding-top: 20px; text-align: center; color: #6b7280; font-size: 14px;">
    ...
    const data = await resend.emails.send({
      ...
      bcc: ["jimmie.castillo@modulex.com", "Chris.Pretorius@modulex.com"],
    ...
    try {
      ...
      console.log("[v0] Order logged to database", { requestNumber })
    } catch (dbError) {
      console.error("[v0] Failed to log order to database:", dbError)
